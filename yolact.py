    ...
    def make_priors(self, conv_h, conv_w, device):
        ...
        with timer.env('makepriors'):
            if self.last_img_size != (cfg._tmp_img_w, cfg._tmp_img_h):
                ...
                self.priors = torch.tensor(prior_data, dtype=torch.float32, device=device).view(-1, 4).detach()
    ...
    def __init__(self):
        ...
        if cfg.mask_type == mask_type.direct:
            ...
            if cfg.mask_proto_use_grid:
                self.grid = torch.tensor(np.load(cfg.mask_proto_grid_file), dtype=torch.float32)
